...
export const questionReducer = (
  state: QuizStateTypeReducer = initialState,
  action: ActionType
): QuizStateTypeReducer => {
  switch (action.type) {
    case 'CHECK-RESULT': {
    ...
    case 'REMOVE-ANSWERED-QUESTION': {
      return {
        ...state,
        quizState: state.quizState.filter(el => el.id !== action.payload.id),
      } // удаляю из стейта отвеченный вопрос чтобы он больше не попался
    }
    default: {
      return state
    }
